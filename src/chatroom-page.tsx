...
function MessageItem({ item, participants, t }: {
  ...
  return <article className="cx-chatroom-message" data-role={item.author.role} aria-live={item.ariaLive}>
    {!human && <ChatroomAvatar participant={{
    ...
    <div className="cx-chatroom-message__content">
      ...
      <div className="cx-chatroom-message__meta">
        <time dateTime={item.timestamp}>{new Date(item.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</time>
        {item.deliveryState === 'failed' && <span>{t('timeline.delivery.failed')}</span>}
        {item.runState === 'running' && <span>{t('timeline.run.running')}</span>}
      </div>
      {item.reactions.length === 0 ? null : <div className="cx-chatroom-message__reactions">
        {item.reactions.map(reaction => {
          ...
          return <span key={reaction.reactionId} data-state={reaction.state}>
            <ChatroomAvatar participant={actor ?? {
...
function StatusItem({ item, participant, t }: {
  ...
  if (item.kind === 'status') {
    return <div className="cx-chatroom-status" data-state={item.state} aria-live={item.ariaLive}>
      {display(item.label, t)}
    </div>;
  }
  return <div className="cx-chatroom-status cx-chatroom-status--member" data-state={item.state}>
    <ChatroomAvatar participant={participant ?? { id: item.participantId, name: item.participantId }} />
    <span>{participant?.name ?? item.participantId} · {t('timeline.member.presence', { state: item.state })}
      {item.diagnostic === undefined ? null : ` · ${display(item.diagnostic, t)}`}</span>
  </div>;
}
...
function approvalReason(item: Extract<ChatroomPageItem, { readonly kind: 'approval' }>, t: Translate): string {
  if ('reason' in item) {
    ...
    if (item.reason !== null && typeof item.reason === 'object' && 'text' in item.reason
      && typeof item.reason.text === 'string') return item.reason.text;
...
function ApprovalItem({ item, participant, roomId, source, t }: {
  ...
  return <article className="cx-chatroom-approval" data-state={item.state}>
    <header>
      <ChatroomAvatar participant={participant ?? { id: item.participantId, name: item.participantId }} />
      <div>
    ...
    {item.state !== 'pending' ? null : <div className="cx-chatroom-approval__actions">
      ...
      {hasCancel && <Button type="button" variant="ghost" disabled={busy} onClick={() => void decide('cancelled')}>
        {t('approval.cancel')}
      </Button>}
    </div>}
...
function Timeline({ items, participants, roomId, source, t }: {
  ...
  return <section className="cx-chatroom-timeline" aria-label={t('timeline.label')}>
    {items.length === 0
...
function Composer({ roomId, source, shortcutPolicy, navigation, signal, t }: {
  ...
  return <form className="cx-chatroom-composer" onSubmit={onSubmit}>
    <textarea
      value={draft}
      rows={1}
      maxLength={32_768}
...
function ChatroomPage({ source, imageCache, ...props }: CordisXReactPageProps & {
  ...
  useEffect(() => { void source.hydrate(roomId); }, [roomId, source]);
  const participants = useMemo(() => snapshot.participants.map(participant => ({
  ...
  const capture = useMemo<ChatroomSidebarImageCapture | undefined>(() => snapshot.room === undefined
    ? undefined
    : imageCache.begin(snapshot.room.id, avatarFingerprint),
  [avatarFingerprint, imageCache, snapshot.room?.id]);
  const publishSnapshot = useCallback((image: Parameters<ChatroomSidebarImageCapture['publish']>[0]) => {
    capture?.publish(image);
  }, [capture]);

  if (snapshot.missing) {
    return <div className="cx-chatroom-page">
  ...
  return <div className="cx-chatroom-page">
    ...
    <header className="cx-chatroom-header">
      <ChatroomCompositeAvatar participants={participants} size="header" onSnapshot={capture === undefined ? undefined : publishSnapshot} />
        ...
        <ul>
          {participants.map(participant => {
            ...
            return <li key={participant.id}>
              ...
              <span><strong>{participant.name}</strong><small>{active === undefined
                ? props.t('members.status.idle')
                : props.t(`members.status.${active.lifecycle.phase}`)}</small></span>
