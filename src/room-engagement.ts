import type { AgentLoopTaskBinding } from '@cordisx/protocol/agent-loop/v2';
...
export function failRoomRunPresence(
  ...
  if (run.presence.state === 'failed'
    ...
    && run.presence.failure.retryCommand?.commandId === input.retryCommand?.commandId) return room;
  return replaceRoomRun(room, runId, {
    ...run,
    status: 'failed',
    presence: {
