...
function Fact({ label, children }: FactProps) {
  return <div className="cx-team-architecture__fact">
    <dt>{label}</dt>
    <dd>{children}</dd>
  </div>;
}
...
function StringList({ values, empty }: { readonly values: readonly string[]; readonly empty: string }) {
  ...
  return <ul className="cx-team-architecture__tokens">
...
function TreeNodeView({ node, onSelect, t }: TreeNodeViewProps) {
  ...
  >
    ...
    {hasChildren && <div className="cx-team-architecture__children" role="list">
      {node.children.map(child => <TreeNodeView
...
function EntityReferenceList({
  ...
  return <ul className="cx-team-architecture__references">
    {memberIds.map(memberId => <li key={memberId}>
...
function EntityDetail({ entity, entities, tab, navigation, t }: {
  ...
  const runtime = [
    ...
    capabilities.runtime.providerId === undefined || capabilities.runtime.modelId === undefined
      ? undefined : `${capabilities.runtime.providerId}/${capabilities.runtime.modelId}`,
  ...
  if (tab === 'overview') {
    panel = <section className="cx-team-architecture__section">
      ...
      <dl className="cx-team-architecture__facts">
        ...
        <Fact label={t('detail.source.revision')}><code>{entity.definitionIdentity.revision}</code></Fact>
      </dl>
    </section>;
  } else if (tab === 'prompts') {
    panel = <section className="cx-team-architecture__section">
        ...
        <Fact label={t('detail.manager')}>
          {entity.relationships.kind === 'unestablished'
            ...
            : managerId === undefined
              ...
              : <Fragment><span>{manager?.label ?? managerId}</span> <code>{managerId}</code></Fragment>}
        </Fact>
        <Fact label={t('detail.direct-reports')}>
          <EntityReferenceList memberIds={entity.relationships.directReportMemberIds} entities={byId}
            empty={t('detail.none')} />
        </Fact>
        <Fact label={t('detail.related')}>
          <EntityReferenceList memberIds={entity.relationships.relatedMemberIds} entities={byId}
            empty={t('detail.none')} />
        </Fact>
    ...
    panel = <section className="cx-team-architecture__section">
      ...
      <dl className="cx-team-architecture__facts">
        ...
        <Fact label={t('detail.skills')}><StringList values={capabilities.skills} empty={t('detail.none')} /></Fact>
        ...
        <Fact label={t('detail.runtime')}><StringList values={runtime} empty={t('detail.none')} /></Fact>
      </dl>
    </section>;
  } else {
    panel = <section className="cx-team-architecture__section">
      ...
      {entity.activeSessions.length === 0
        ...
        : <ul className="cx-team-architecture__sessions">
          {entity.activeSessions.map(session => <li key={`${session.roomId}:${session.runId}`}>
            ...
            <dl className="cx-team-architecture__facts">
              ...
              <Fact label={t('detail.session.run')}><code>{session.runId}</code></Fact>
...
function TeamArchitectureRoot({
  ...
  const model = useMemo(() => buildTeamArchitectureViewModel(snapshot, {
    query,
    role,
    session,
    relationship,
  ...
  return <div className="cx-team-architecture">
    ...
    {model.matchedCount === 0
      ...
      : <div className="cx-team-architecture__groups">
        {model.roots.length > 0 && <div className="cx-team-architecture__chart">
          <div className="cx-team-architecture__chart-scroll" role="region" aria-label={t('tree.heading')} tabIndex={0}>
            <div className="cx-team-architecture__forest" role="list" aria-label={t('tree.heading')}>
              {model.roots.map(node => <TreeNodeView
        ...
        {model.unestablished.length > 0 && <section className="cx-team-architecture__unestablished" aria-labelledby="team-unestablished-heading">
          <h2 id="team-unestablished-heading">{t('tree.unestablished')}</h2>
          <div className="cx-team-architecture__chart-scroll" role="region" aria-label={t('tree.unestablished')} tabIndex={0}>
            <div className="cx-team-architecture__forest" role="list" aria-label={t('tree.unestablished')}>
              {model.unestablished.map(node => <TreeNodeView
                key={node.entity.memberId}
                node={node}
                onSelect={select}
                t={t}
...
function detailTabForRoute(
  ...
  const matches = (localRouteId: string) =>
    routeId === localRouteId || routeId.endsWith(`:${localRouteId}`);
