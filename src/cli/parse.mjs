const usage =
  'cordisx-chatroom --binding <path> send|delegate|query|recover --operation <id> [--text <text>] [--to <member>] [--cwd <absolute-path>] [--project <id>] [--room <id>]';
...
export function parseChatroomArguments(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (['send', 'delegate', 'query', 'recover'].includes(arg) && command === undefined) {
      command = arg;
      continue;
    }
    if (
  ...
  const allowed = ['query', 'recover'].includes(command) ? common : command === 'delegate'
  ...
  if (
    ...
    || !['query', 'recover'].includes(command) && (!values.get('--text')?.trim() || values.get('--text').length > 16000)
  ...
  return {
    ...
    input: {
      ...
      ...(['query', 'recover'].includes(command) ? {} : { text: values.get('--text') }),
