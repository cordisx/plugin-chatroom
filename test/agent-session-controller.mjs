...
test('a terminal V7 rejection retains the complete Room projection through same-Session route replacement', async () => {
  ...
  assert.ok(lead);
  const a = {
    kind: 'message', itemId: 'room-a', messageId: 'room-a-message', sequence: 1,
    source: 'agent-loop',
    author: {
      participantId: human.id, role: 'human',
    ...
    reactions: [], timestamp: aTimestamp, deliveryState: 'pending', runState: 'idle', ariaLive: 'off', actions: [],
  };
  const delegation = {
    kind: 'message', itemId: 'lead-delegation-terminal', messageId: 'lead-delegation-terminal', sequence: 2,
    source: 'chatroom-acknowledgement',
    author: {
      participantId: lead.participantId, role: 'agent',
    ...
    reactions: [], timestamp: delegationTimestamp, deliveryState: 'delivered', runState: 'idle', ariaLive: 'polite', actions: [],
  };
  const b = {
    kind: 'message', itemId: 'room-b', messageId: 'room-b-message', sequence: 3,
    source: 'agent-loop',
    author: {
      participantId: human.id, role: 'human',
      displayName: { namespace: 'chatroom', key: 'participant.name', fallback: human.name },
    ...
    reactions: [], timestamp: bTimestamp, deliveryState: 'pending', runState: 'idle', ariaLive: 'off', actions: [],
  };
  room = createRoom({
    ...room,
    participants: [
  ...
  const reviewerReplay = [
    ...
    sessionEvent(reviewerSessionId, 2, 'assistant/message', {
      turn: 1, step: 1,
      message: {
        id: 'reviewer-introduction', role: 'assistant',
  ...
  const decision = controller.requestApproval(
    'room', 'review-run', 'shell', 'Reviewer needs permission to inspect the protected result.', 'call-reject',
  );
  await new Promise(resolve => setImmediate(resolve));
  ...
  const linkedRoom = recordRoomAdmissionMessageLink(store.rooms.get('room'), {
    roomId: 'room', itemId: b.itemId,
    participantId: lead.participantId, memberId: lead.memberId, runId: 'lead-run',
    sessionId: leadSessionId, messageId: 'message-b', owner,
  ...
  const leadBEvents = [
    ...
    sessionEvent(leadSessionId, 1, 'user/message', {
      id: 'message-b', role: 'user', content: [{ type: 'text', text: '1' }],
  ...
  const binding = {
    bindingId: 'binding-terminal-reject', shell: 'agent-desktop', ownerGeneration: 'owner-terminal-reject',
    routeSelection: { scope: 'room-or-new', selectedRoomParam: 'room' },
  };
  const domain = new ChatroomConversationController(store.rooms);
  ...
  const rejectionResult = sessionEvent(reviewerSessionId, 6, 'assistant/message', {
    turn: 1, step: 2,
    message: {
      id: 'reviewer-rejection-result', role: 'assistant',
  ...
  const approvalEvents = [
    sessionEvent(reviewerSessionId, 3, 'approval/authority-bound', {
      approvalId: 'approval-v2-1', requester: reviewer.definition, authority: lead.definition,
    ...
    sessionEvent(reviewerSessionId, 4, 'approval/asked', {
      id: 'approval-v2-1', toolName: 'shell', callId: 'call-reject',
  ...
  const rejectionItem = terminal.items.find(item => item.kind === 'message'
...
test('Shell v9 bootstrap target is issued before first exact Agent acquisition and reservation submit', async () => {
  ...
  room = addRoomRun(room, {
    runId: 'lead-bootstrap', memberId: 'leader', title: 'Lead', status: 'creating',
  });
  ...
  const origin = {
    ...
    originId: 'origin-v9', binding: { bindingId: 'binding-v9', ownerGeneration: 'owner-v9' },
    generation: 'shell-v9', executionId: 'execution-v9', commandId: CHATROOM_COMMAND_SUBMIT,
  ...
  const outcomes = await controller.submitDeliveriesViaAdmissionV4(
    room.id, [{ memberId: 'leader', runId: 'lead-bootstrap' }], origin,
    'Start the first Room task.', targets, reservations,
  );
  ...
  assert.deepEqual(outcomes, [{
    ...
    outcome: {
      status: 'accepted', roomId: room.id, runId: 'lead-bootstrap', messageId: 'host-v9-message',
...
test('Shell v9 bootstrap denial fails the freshly persisted run closed before acquire or reserve', async () => {
  ...
  room = addRoomRun(room, {
    runId: 'lead-bootstrap-denied', memberId: 'leader', title: 'Lead', status: 'creating',
  });
  const harness = runtimeHarness({ room });
  const store = DurableChatroomRoomStore.memory([room]);
  const controller = new ChatroomAgentSessionController(
  ...
  let reserves = 0;
  const outcomes = await controller.submitDeliveriesViaAdmissionV4(
    room.id, [{ memberId: 'leader', runId: 'lead-bootstrap-denied' }], {
      $schema: 'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/agent-bootstrap-command-origin.v1.schema.json',
      contract: 'cordisx.agent-bootstrap-command-origin/v1', schemaVersion: 1,
      originId: 'origin-v9-denied', binding: { bindingId: 'binding-v9', ownerGeneration: 'owner-v9' },
      generation: 'shell-v9', executionId: 'execution-v9', commandId: CHATROOM_COMMAND_SUBMIT,
      scope: 'composer-submit',
    },
...
test('Shell v9 route declaration binds the exact persisted Room before first Agent acquisition and reservation submit', async () => {
  ...
  const origin = {
    $schema: 'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/agent-bootstrap-command-origin.v1.schema.json',
    contract: 'cordisx.agent-bootstrap-command-origin/v1', schemaVersion: 1,
    originId: 'origin-v9-route', binding: { bindingId: 'binding-v9-route', ownerGeneration: 'owner-v9-route' },
    generation: 'shell-v9', executionId: 'execution-v9-route', commandId: CHATROOM_COMMAND_SUBMIT,
    scope: 'composer-submit',
  };
  const declarations = {
    declare: async request => {
      order.push('declare');
      assert.equal(harness.creates.length, 0, 'route declaration binds before acquire/create');
      assert.deepEqual(request, {
        origin,
        target: {
          roomId: room.id,
          ...
          runId: 'lead-bootstrap-route',
          route: { routeId: 'room', param: 'roomId', roomId: room.id },
        },
      });
      return {
        status: 'declared',
  ...
  const reservations = {
    reserve: async request => {
      ...
      assert.equal(harness.creates.length, 1, 'reserve receives the exact newly acquired Agent handle');
      assert.equal(request.handle, harness.handles[0].handle);
      assert.equal(request.continuation.token, 'route-continuation-lead');
      assert.deepEqual(request.message, { text: 'Start the first Room task.' });
      return {
        status: 'reserved',
        reservation: {
          reservationId: 'bootstrap-route-reservation-lead',
          submit: async () => {
            order.push('submit');
            return admission('host-v9-route-message');
          },
          revoke: async () => {},
        },
      };
  ...
  assert.deepEqual(outcomes, [{
    memberId: 'leader', runId: 'lead-bootstrap-route',
    outcome: {
      status: 'accepted', roomId: room.id, runId: 'lead-bootstrap-route', messageId: 'host-v9-route-message',
      sessionId: 'session-created-1', disposition: 'created',
    },
  }]);
...
test('Shell v9 route declaration denial fails the persisted run closed before acquire or reserve', async () => {
  let room = createRoom({ id: 'room-bootstrap-route-denied', title: 'Bootstrap Route Room' });
  room = addRoomRun(room, {
    runId: 'lead-bootstrap-route-denied', memberId: 'leader', title: 'Lead', status: 'creating',
  });
  const harness = runtimeHarness({ room });
  const store = DurableChatroomRoomStore.memory([room]);
  const controller = new ChatroomAgentSessionController(
  ...
  const outcomes = await controller.submitDeliveriesViaAdmissionV6(
    ...
    'Do not acquire without a bootstrap route declaration.',
    { declare: async () => ({ status: 'denied', code: 'route-denied' }) },
    { reserve: async () => { reserves += 1; throw new Error('must not reserve'); } },
  );

  assert.deepEqual(outcomes, [{
    memberId: 'leader', runId: 'lead-bootstrap-route-denied',
    outcome: { status: 'denied', roomId: room.id, runId: 'lead-bootstrap-route-denied', code: 'route-denied' },
  }]);
  assert.equal(harness.creates.length, 0);
  assert.equal(reserves, 0);
  assert.equal(store.document(room.id)?.room.runs.find(run => run.runId === 'lead-bootstrap-route-denied')?.status, 'failed');
  await controller.dispose();
  store.dispose();
});
    ...
    issue: async request => {
      ...
      return {
        ...
        origin: {
          ...
          contract: 'cordisx.agent-admission-target-origin/v3', schemaVersion: 3, token: 'opaque-review-run',
        },
      };
    },
  };
