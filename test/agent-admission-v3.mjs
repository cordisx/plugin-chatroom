...
const baseOrigin = Object.freeze({
  ...
  scope: 'composer-submit', room: {
    roomId: 'room-1', participantId: 'room', memberId: 'room', runId: 'room',
  },
});
      ...
      issue: async request => {
        ...
        return {
          ...
          origin: Object.freeze({
            ...
            contract: 'cordisx.agent-admission-target-origin/v3', schemaVersion: 3,
...
async function submitMany(count) {
  ...
  const results = await Promise.all(Array.from({ length: count }, async (_, index) => {
    ...
    return await submitChatroomAgentAdmissionV3(service.origins, service.reservations, {
      handle: handle(suffix), origin: baseOrigin, target: target(suffix), message: { text: 'Review this exact change.' },
    });
  }));
...
test('v3 adapter issues and reserves independent opaque capabilities for N=3 deliveries', async () => {
  ...
  assert.deepEqual(results.map(result => result.admission.messageId), [
    'host-message-opaque-run-1', 'host-message-opaque-run-2', 'host-message-opaque-run-3',
  ]);
});
...
test('v3 adapter stops after an issue denial and never reserves or directly dispatches', async () => {
  ...
  const reservations = { reserve: async () => { calls.reserved += 1; throw new Error('must not reserve'); } };

  const result = await submitChatroomAgentAdmissionV3(origins, reservations, {
    handle: handle('denied'), origin: baseOrigin, target: target('denied'), message: { text: 'Review this exact change.' },
  });
...
test('v3 adapter stops after a reservation denial and never directly dispatches', async () => {
  const origins = { issue: async () => ({ status: 'issued', origin: {
    $schema: 'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/agent-admission-target-origin.v3.schema.json',
    contract: 'cordisx.agent-admission-target-origin/v3', schemaVersion: 3, token: 'opaque-denied',
  } }) };
