...
const routingQuestion = room => {
  ...
  const registration = {
    ...
    owner: {
      pluginId: 'chatroom',
      installationId: 'chatroom-installation',
  ...
  return {
    ...
    schemaVersion: 1,
    routingId: 'routing-reviewer-one',
    registration,
    requester,
    toolName: 'shell',
    callId: 'call-review',
...
test('prepares an approval/v2 request from exact Reviewer and reportsTo Lead live bindings', () => {
  ...
  assert.deepEqual(result.request.reason, {
    kind: 'plain-text',
    text: 'Reviewer needs permission to inspect the exact diff.',
  });
...
test('calls approval/v2 once with exact targets and fences the returned requester/authority bindings', async () => {
    ...
    request: async request => {
      ...
      return {
        ...
        requester: {
          ...
          agentGeneration: reviewer.generation,
          definition: request.requester.definition,
        },
        authority: {
          agentId: lead.id,
          sessionId: lead.session.id,
          agentGeneration: lead.generation,
          definition: request.authority.definition,
        },
  ...
  assert.deepEqual(mismatched, {
    status: 'unavailable',
    code: 'decision-correlation-invalid',
  });
});
...
test('driver approval routing fails closed for stale requester registration or absent live Lead', () => {
  ...
  assert.equal(route(question, runId => runId === 'reviewer-run' ? reviewer : undefined).code, 'authority-unavailable');
  assert.equal(
    route(question, runId => runId === 'reviewer-run' ? reviewer : fakeAgent('foreign-session', 11)).code,
...
test('request preparation fails closed for missing hierarchy, non-exact identity, or non-exact live Agent', () => {
  ...
  assert.deepEqual(prepare({ room: withoutReportsTo }), {
    status: 'unavailable',
    code: 'authority-member-unavailable',
  });
  ...
  assert.deepEqual(prepare({ room: wildcardIdentity }), {
    status: 'unavailable',
    code: 'authority-identity-unavailable',
  });
});
...
test('maps every durable v7 decision to an actionless terminal state in the same item slot', () => {
  ...
  for (const [outcome, state] of expected) {
    ...
    assert.equal(result.item.itemId, chatroomApprovalBubbleItemId('session-reviewer', 'approval-review'));
...
test('keeps old v6 ledgers on the compatibility path and fails closed on divergent v7 facts', () => {
  ...
  const wrongReason = authorityEvents(room).map(event =>
